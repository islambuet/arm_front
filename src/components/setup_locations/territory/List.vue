<template>
  <div>
    <div v-show="$system_variables.status_data_loaded==1">
      <div class="card d-print-none mb-2">
        <div class="card-body">          
          <router-link to="/sys_module_task/add" :class="'btn btn-success mr-2 mb-2'"  v-if="$parent.permissions.action_1">{{$system_variables.get_label('button_new')}}</router-link>            
      ...
      <div class="card d-print-none mb-2" v-if="show_column_controls">
        <div class="card-body">
            <ColumnControl :controller="'Area'" :method="'list'" :control_columns="$parent.columns.control_columns" :hidden_columns="$parent.columns.hidden_columns" :on_change_control="$parent.set_display_columns"/>            
        </div>
      </div>
      <div class="card mb-2">
            ...
            <b-table  
              ...
              <template v-slot:cell(actions)="data">
                <b-dropdown size="sm" :text="$system_variables.get_label('button_action')" dropright variant="primary" :class="'action_menu'">
                  <b-dropdown-item :to="'/setup_locations/area/edit/'+data.item.id">{{$system_variables.get_label('action_edit')}}</b-dropdown-item>
