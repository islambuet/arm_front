  ...
  <div>
    <div v-if="$parent.permissions.action_1 || $parent.permissions.action_2" v-show="$system_variables.status_data_loaded==1">
        <div class="card d-print-none mb-2">
            <div class="card-body">
              <router-link  to="/setup_designation" :class="'btn btn-success mr-2 mb-2'" >{{$system_variables.get_label('action_back')}}</router-link>            
        ...
        <div class="card d-print-none mb-2">
          ...
          <div class="card-body">
            <form id="form_save">
              ...
              <b-row class="mb-2">
                <b-col cols="4" class="text-right"><label>{{$system_variables.get_label_task('label_parent_name')}} <strong class="text-danger"> * </strong></label></b-col>
                <b-col cols="8" sm="4">
                  <select class="form-control" v-model="$parent.item.parent" name="item[parent]" required>
                        <option value="0">{{$system_variables.get_label('Label_select')}}</option>
                        <option v-for="(option, index) in $parent.designations" :key="index" :value="option.id">
                            {{ option.name}}
                        </option>
                    </select>
                </b-col>                
              ...
              <b-row class="mb-2">
                <b-col cols="4" class="mt-1 text-right"><label>{{$system_variables.get_label('label_ordering')}}</label> <strong class="text-danger"> * </strong></b-col>
                <b-col cols="8" sm="4">
                  <input type="number" name="item[ordering]" v-model="$parent.item.ordering" class="form-control" min="0" required/>
                </b-col>                
              </b-row>
              <b-row class="mb-2">
...
export default {
  ...
  methods:{
    save:function(save_and_new)
    {
      ...
      this.$axios.post('/setup_designation/save_item',form_data)
      .then(response=>{          
        ...
        else
        {
          ...
          if(save_and_new)
          {
            this.$router.push("/setup_designation/add");
          }
          else
          {
