...
export default {
  name: "SetupSystemConfiguration",
  components: { List, AddEdit },
  mounted: function () {
    this.$system_variables.labels_task = this.$system_functions.load_languages([
        {language:this.$system_variables.language, file:'components/setup_system_configuration/language.js'},
    ]);
    this.init();
  },
  data() {
    return {
      ...
      pagination:{
        ...
        num_item_showing:0,
        num_items:10,
        page_options: [10, 20, 30, 50, 100, 500]
      }, 
  ...
  methods: {
    routing: function (route) {
      if (route.path == "/setup_system_configuration") {
        this.method = "list";
        this.get_items();
      } else if (route.path == "/setup_system_configuration/add") {
        this.method = "add";
        this.add_edit(0);
      } else if (route.path.indexOf("/setup_system_configuration/edit/") != -1) {
        this.method = "edit";
    ...
    init: function () {
      ...
      this.$axios.post("/setup_system_configuration/initialize", form_data)
      .then((response) => {
        if(response.data.error_string == '')
        {
          ...
          this.method_switching(this.$route);
        }
        else
        {
        ...
        if (response.data.error_type) 
        {
          this.$system_variables.status_task_loaded = 0;
          this.$bvToast.toast(
            this.$system_variables.get_label(response.data.error_type), // Content
            {
              title: this.$system_variables.get_label("label_error"), // Title
              variant: "danger",
              autoHideDelay: 5000,
              appendToast: false,
            }
        ...
        else 
        {
          ...
          if (response.data.max_module_task_action) {
            this.max_module_task_action =
              response.data.max_module_task_action;
          }
          this.max_modules_tasks_level =
            response.data.max_modules_tasks_level;
    ...
    set_control_columns: function () {
      ...
      var control_columns = [];
      control_columns.push({
        label: this.$system_variables.get_label("label_id"),
        value: "id",
        selected: this.columns.hidden_columns.indexOf("id") >= 0 ? "id" : "",
      });
      control_columns.push({
        label: this.$system_variables.get_label("label_ordering"),
        value: "ordering",
        selected:
          this.columns.hidden_columns.indexOf("ordering") >= 0
            ? "ordering"
            : "",
      });
      control_columns.push({
        label: this.$system_variables.get_label("label_status"),
        value: "status",
        selected:
          this.columns.hidden_columns.indexOf("status") >= 0 ? "status" : "",
      });
    ...
    set_filter_columns: function () {
      ...
      filter_columns["id"] = {
        filter_type: "number",
        fitler_from: { value: "", default_value: "" },
        fitler_to: { value: "", default_value: "" },
      };
    ...
    set_display_columns: function () {
      var columns = {};
      columns["actions"] = {
        label: this.$system_variables.get_label("label_action"),
        hidden: this.columns.false,
        sticky_column: false,
      }; //cannot set it sticky becuase dropdown will goes bellow
      columns["id"] = {
        label: this.$system_variables.get_label("label_id"),
        hidden: this.columns.hidden_columns.indexOf("id") >= 0 ? true : false,
        sticky_column: true,
        sortable: false,
      };
      columns["name"] = {
        label: this.$system_variables.get_label("label_name"),
        hidden: this.columns.hidden_columns.indexOf("name") >= 0 ? true : false,
        sticky_column: false,
        sortable: false,
      };
      columns["num_tasks"] = {
        label: this.$system_variables.get_label("label_num_tasks"),
        hidden:
          this.columns.hidden_columns.indexOf("num_tasks") >= 0 ? true : false,
        sticky_column: false,
        sortable: false,
      };
      columns["ordering"] = {
        label: this.$system_variables.get_label("label_ordering"),
        hidden:
          this.columns.hidden_columns.indexOf("ordering") >= 0 ? true : false,
        sticky_column: false,
        sortable: false,
      };
      columns["status"] = {
        label: this.$system_variables.get_label("label_status"),
        hidden:
          this.columns.hidden_columns.indexOf("status") >= 0 ? true : false,
        sticky_column: false,
        sortable: false,
      };
      this.columns.display_columns = this.$system_functions.get_display_columns(
        columns
      );
    },
    get_items: function () {
      if (this.reload_items) {
        ...
        form_data.append("token_auth", this.$system_variables.user.token_auth);
        this.$axios
          .post("/sys_user_group/get_items", form_data)
          .then((response) => {
            this.$system_variables.status_data_loaded = 1;
            if (response.data.error_type) {
              this.$bvToast.toast(
                this.$system_variables.get_label(response.data.error_type),
                {
          ...
          .catch((error) => {
            this.$system_variables.status_data_loaded = 1;
            this.$bvToast.toast(
              this.$system_variables.get_label("Response Error"),
              {
                title: this.$system_variables.get_label("label_error"),
                variant: "danger",
                autoHideDelay: 5000,
    ...
    add_edit: function (item_id) {
      if (item_id > 0) {
        if (!this.permissions.action2) {
          ...
          form_data.append(
            "token_auth",
            this.$system_variables.user.token_auth
          );
          form_data.append("item_id", item_id);
          this.$axios
            .post("/sys_user_group/get_item", form_data)
            .then((response) => {
              this.$system_variables.status_data_loaded = 1;
              if (response.data.error_type) {
                this.$bvToast.toast(
                ...
                if (response.data.item) {
                  ...
                  this.$bvToast.toast(
                    this.$system_variables.get_label("Data Not Found"),
                    {
                      title: this.$system_variables.get_label("label_error"),
                      variant: "danger",
                      autoHideDelay: 5000,
