...
export default {
    ...
    mounted:function()
    {
       this.$system_variables.labels_task=this.$system_functions.load_languages([
        {language:this.$system_variables.language,file:'components/setup_product_crop/language.js'}
    ...
    {      
      if(route.path=='/setup_product_crop')
      {
        this.method='list';
        this.get_items();
      }
      else if(route.path=="/setup_product_crop/add")
      {
        this.method='add';
        this.add_edit(0);
      }
      else if(route.path.indexOf("/setup_product_crop/edit/")!=-1)
      {
        this.method='edit';        
    ...
    init:function()
    {
        ...
        this.$axios.post('/setup_product_crop/initialize',form_data)
        .then(response=>{
    ...
    get_items:function()
    {
        if(this.reload_items)
        {
            ...
            //form_data.append ('token_auth', this.$system_variables.user.token_auth);                  
            form_data.append ('token_auth', localStorage.getItem('token_auth'));                  
            this.$axios.post('/setup_product_crop/get_items',form_data)
            .then(response=>{          
              this.$system_variables.status_data_loaded=1;
              if(response.data.error_type)        
              {   
    ...
    add_edit:function(item_id)
    {
      if(item_id>0)
      {
        ...
        else
        {
          ...
          this.$axios.post('/setup_product_crop/get_item',form_data)
          .then(response=>{          
            this.$system_variables.status_data_loaded=1;
            if(response.data.error_type)        
            {            
