  ...
  <div>
    <div v-if="$parent.permissions.action_0" v-show="$system_variables.status_data_loaded==1">
      ...
      <div class="card d-print-none mb-2" v-if="show_column_controls">
        <div class="card-body">
            <ColumnControl :controller="'setup_system_configures'" :method="'list'" :control_columns="$parent.columns.control_columns" :hidden_columns="$parent.columns.hidden_columns" :on_change_control="$parent.set_display_columns"/>            
        </div>
      </div>
      <div class="card mb-2">
          <div class="card-header d-print-none">
              {{$system_variables.get_label_task('label_sys_system_configuration_list')}}
          </div>
          <div class="card-body"> 
            <b-table  
              ...
              <template v-slot:head(purpose)="data">              
                <div>{{data.label}}</div>
                <div v-if="show_fitler_options" class="d-print-none">
                  <input type="text"  class="form-control" v-model="$parent.columns.filter_columns['purpose'].value" />
                </div>
              </template>
              ...
              <template v-slot:head(config_value)="data">              
                <div>{{data.label}}</div>
                <div v-if="show_fitler_options" class="d-print-none">
                  <input type="text"  class="form-control" v-model="$parent.columns.filter_columns['config_value'].value" />
                </div>
              </template>
              ...
              <template v-slot:cell(actions)="data">
                <b-dropdown size="sm" :text="$system_variables.get_label('button_action')" dropright variant="primary" :class="'action_menu'">
                  ...
                  <b-dropdown-item :to="'/setup_system_configures/role/'+data.item.id">{{$system_variables.get_label_task('action_assign_role')}}</b-dropdown-item> -->
                </b-dropdown>
              </template>
            </b-table>
          </div>          
