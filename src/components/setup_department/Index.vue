...
export default {
    ...
    mounted:function()
    {
       this.$system_variables.labels_task=this.$system_functions.load_languages([
        {language:this.$system_variables.language,file:'components/setup_department/language.js'}
    ...
    {      
      if(route.path=='/setup_department')
      {
        this.method='list';
        this.get_items();
      }
      else if(route.path=="/setup_department/add")
      {
        this.method='add';
        this.add_edit(0);
      }
      else if(route.path.indexOf("/setup_department/edit/")!=-1)
      {
        this.method='edit';        
    ...
    init:function()
    {
        ...
        this.$axios.post('/setup_department/initialize',form_data)
        .then(response=>{
    ...
    get_items:function()
    {
        if(this.reload_items)
        {
            ...
            //form_data.append ('token_auth', this.$system_variables.user.token_auth);                  
            form_data.append ('token_auth', localStorage.getItem('token_auth'));                  
            this.$axios.post('/setup_department/get_items',form_data)
            .then(response=>{          
              this.$system_variables.status_data_loaded=1;
              if(response.data.error_type)        
              {   
    ...
    add_edit:function(item_id)
    {
      if(item_id>0)
      {
        ...
        else
        {
          ...
          this.$axios.post('/setup_department/get_item',form_data)
          .then(response=>{          
            this.$system_variables.status_data_loaded=1;
            if(response.data.error_type)        
            {            
