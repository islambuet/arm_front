    ...
    <div v-if="$parent.permissions.action_0" v-show="$system_variables.status_data_loaded==1">
      <div class="card d-print-none mb-2">
        <div class="card-body">
            ...
            <router-link to="/setup_product_crop_type/add" :class="'btn btn-success mr-2 mb-2'">{{$system_variables.get_label('button_new')}}</router-link>            
      ...
      <div class="card d-print-none mb-2" v-if="show_column_controls">
        <div class="card-body">
            <ColumnControl :controller="'setup_product_crop_type'" :method="'list'" :control_columns="$parent.columns.control_columns" :hidden_columns="$parent.columns.hidden_columns" :on_change_control="$parent.set_display_columns"/>            
        </div>
      </div>
      <div class="card mb-2">
          ...
          <div class="card-body"> 
            <b-table  
              ...
              <template v-slot:head(crop_name)="data">              
                <div>{{data.label}}</div>
                <div v-if="show_fitler_options" class="d-print-none">
                  <input type="text"  class="form-control" v-model="$parent.columns.filter_columns['crop_name'].value" />
                </div>
              </template>
              ...
              <template v-slot:head(description)="data">              
                <div>{{data.label}}</div>
                <div v-if="show_fitler_options" class="d-print-none">
                  <input type="text"  class="form-control" v-model="$parent.columns.filter_columns['description'].value" />
                </div>
              </template>
              ...
              <template v-slot:cell(actions)="data">
                <b-dropdown size="sm" :text="$system_variables.get_label('button_action')" dropright variant="primary" :class="'action_menu'">
                  <b-dropdown-item :to="'/setup_product_crop_type/edit/'+data.item.id">{{$system_variables.get_label('action_edit')}}</b-dropdown-item>
                  <!-- <b-dropdown-divider></b-dropdown-divider>
                  <b-dropdown-item :to="'/setup_product_crop_type/role/'+data.item.id">{{$system_variables.get_label_task('action_assign_role')}}</b-dropdown-item> -->
                </b-dropdown>
              </template>
            </b-table>
          </div>          
