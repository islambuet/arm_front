    ...
    <div v-if="$parent.permissions.action0" v-show="$system_variables.status_data_loaded==1">
      <div class="card d-print-none mb-2">
        <div class="card-body">
            ...
            <b-button @click="$system_functions.export_csv(get_csv_headers,modified_items)" class="mr-2 mb-2" variant="success" v-if="$parent.permissions.action8" >{{$system_variables.get_label('action_csv')}}</b-button>          
            <b-button class="mr-2 mb-2" variant="success" v-if="$parent.permissions.action7" :pressed.sync="show_column_controls">{{$system_variables.get_label('action_column_control')}}</b-button>
            <b-button class="mr-2 mb-2" variant="success" v-if="$parent.permissions.action8" :pressed.sync="show_fitler_options">{{$system_variables.get_label('action_search')}}</b-button>
          ...
          <div class="card-body"> 
            <b-table                          
              ...
              <template v-slot:cell(name)="data">  
                <b-dropdown size="sm" :text="data.value" left variant="outline-primary">
                  <b-dropdown-item :to="'/sys_user_group/edit/'+data.item.id">{{$system_variables.get_label('action_edit')}}</b-dropdown-item>
                  <b-dropdown-divider></b-dropdown-divider>
                  <b-dropdown-item :to="'/sys_user_group/assign_role/'+data.item.id">{{$system_variables.get_label('action_assign_role')}}</b-dropdown-item>
                </b-dropdown>
              </template>
            </b-table>
          </div>          
